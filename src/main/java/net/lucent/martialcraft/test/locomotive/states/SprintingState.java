...
public class SprintingState extends GroundedMovementState {
    ...
    public void enterState(LivingEntity entity, StateData data) {
        MartialCraft.LOGGER.debug("STARTED SPRINTING ("+(entity.level().isClientSide() ? "Client" : "Server")+")");
    }

    @Override
    public void leaveState(LivingEntity entity, StateData data) {
        MartialCraft.LOGGER.debug("STOPPED SPRINTING ("+(entity.level().isClientSide() ? "Client" : "Server")+")");
    }
}